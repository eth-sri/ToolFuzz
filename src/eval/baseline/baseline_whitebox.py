...
def failure_test(tools, model):
    """
        Function which will generate the results for the white box baseline for runtime failures.
    ...
    for tool in tqdm(tools, desc="Testing tools"):
        ...
        save_test_results(failures, 'baseline_whitebox_failure_arxiv.json')
...
def correct_test(loaders):
    ...
    for loader in tqdm(loaders, desc="Testing tools"):
        ...
        for tool in tqdm(tools, desc="Testing tools"):
            ...
            save_test_results(failures, 'baseline_whitebox_correctness.json')


def main():
    loaders = get_correctness_tool_loaders()
    correct_test(loaders)
